...
擦拭键盘前，可以在菜单里打开 `键盘清洁模式`。开启后，普通键盘输入会被临时屏蔽，鼠标仍然可用；擦完后再次点击菜单里的开关即可恢复键盘输入。

首次使用时，macOS 可能要求授予输入监听或辅助功能权限。这个功能只拦截键盘事件，不记录、保存或上传任何按键内容。清洁模式不会持久化保存，退出或重新启动 App 后默认关闭。
